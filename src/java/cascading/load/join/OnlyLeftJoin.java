...
public class OnlyLeftJoin extends Load
  ...
  public Flow createFlow() throws Exception
    ...
    Tap leftSink = new Hfs( new TextLine(), getOutputPaths()[ 0 ], SinkMode.REPLACE );
    ...
    Pipe[] tails = Pipe.pipes( left );
    Map<String, Tap> sinks = Cascades.tapsMap( tails, Tap.taps( leftSink ) );

    return new FlowConnector( properties ).connect( "left-join", sources, sinks, tails );
    }
